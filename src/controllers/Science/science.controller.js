...
const checkAnsScience = async (req, res) => {
  try {
    ...
    const feedbackData = await getAIFeedbackFromMicroservice(progressArray, "science");

    // 6. Update ScienceScore Document
    await updateScienceScore({
      userId,
      correctAnswers,
      progressArray,
      feedbackData,
    ...
    console.error("Error checking science answers:", error);
